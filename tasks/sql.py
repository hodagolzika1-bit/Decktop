...
def show_skill():
    ...
    if len(data)>0:
        for d in data:
            print(f"user_id = {d[0]} ,name = {d[1]} ,emiil = {d[2]} s,age = {d[3]} ")
    else:
...
def add_skill():
    ...
    ag=input("and your age ").strip()
    cr.execute("INSERT INTO gym (name,emill,age)VALUES (?, ?, ?)", (sk, em, ag))
    print("think you added successfully")
def delete_skill():
